...
def densenet(inputs,
  ...
  with tf.variable_scope(scope, 'densenetxxx', [inputs, num_classes],
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      target_fc1 = slim.conv2d(net, 1000, [1, 1],  normalizer_fn=None,  activation_fn=tf.nn.selu, scope='target_fc1')
      end_points[sc.name + '/target_fc1'] = target_fc1
      target_fc2 = slim.conv2d(target_fc1, 250, [1, 1],  normalizer_fn=None,  activation_fn=tf.nn.selu, scope='target_fc2')
      ...
      end_points[sc.name + '/target_logits'] = net
      target_logits = tf.squeeze(target_logits, [1, 2], name='TargetSpatialSqueeze')
      end_points[sc.name + '/target_spatial_squeeze'] = net

      return target_logits, end_points


def densenet121(inputs, num_classes=1000, data_format='NHWC', is_training=True, scope='densenet121', reuse=None):
...
def densenet161(inputs, num_classes=1000, data_format='NHWC', is_training=True, scope='densenet161', reuse=None):
  return densenet(inputs,
                  num_classes=num_classes, 
                  reduction=0.5,
                  growth_rate=48,
...
def densenet169(inputs, num_classes=1000, data_format='NHWC', is_training=True, scope='densenet169', reuse=None):
  return densenet(inputs,
                  num_classes=num_classes, 
                  reduction=0.5,
                  growth_rate=32,
